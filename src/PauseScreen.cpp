...
void PauseScreen::draw(SDL_Renderer * renderer) {
    ...
    SDL_RenderRect(renderer, &rect_background);

    SDL_RenderTexture(renderer, title, NULL, &rect_completed);
    SDL_RenderTexture(renderer, subtitle, NULL, &rect_completed_subtitle);
}
