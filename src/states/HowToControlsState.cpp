...
void HowToControlsState::draw(SDL_Renderer * renderer) {
    ...
    SDL_FRect rect_movement_text = {(float) this->options->getScreenWidth() / 2.0f - (float) this->options->getShellSize() / 2.0f, (float) this->text_start_y, 0.0f, 0.0f};
    SDL_GetTextureSize(this->text_movement, &rect_movement_text.w, &rect_movement_text.h);
    rect_movement_text.x -= rect_movement_text.w;
    rect_movement_text.y = rect_movement_text.y - rect_movement_text.h / 2.0f + (float) this->options->getShellSize() / 2.0f;
    SDL_RenderTexture(renderer, this->text_movement, NULL, &rect_movement_text);

    SDL_FRect rect_movement = {(float) this->options->getScreenWidth() / 2.0f - (float) this->options->getShellSize() / 2.0f, (float) this->text_start_y, (float) this->options->getShellSize(), (float) this->options->getShellSize()};
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::KEYBOARD_MOVEMENT], NULL, &rect_movement);
    rect_movement.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::MOUSE_MOVEMENT], NULL, &rect_movement);
    rect_movement.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::GAMEPAD_MOVEMENT1], NULL, &rect_movement);
    rect_movement.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::GAMEPAD_MOVEMENT2], NULL, &rect_movement);

    SDL_FRect rect_confirm_text = {(float) this->options->getScreenWidth() / 2.0f - (float) this->options->getShellSize() / 2.0f, rect_movement.y + rect_movement.h, 0.0f, 0.0f};
    SDL_GetTextureSize(this->text_confirm, &rect_confirm_text.w, &rect_confirm_text.h);
    rect_confirm_text.x -= rect_confirm_text.w;
    rect_confirm_text.y = rect_confirm_text.y - rect_confirm_text.h / 2.0f + (float) this->options->getShellSize() / 2.0f;
    SDL_RenderTexture(renderer, this->text_confirm, NULL, &rect_confirm_text);

    SDL_FRect rect_confirm = {(float) this->options->getScreenWidth() / 2.0f - (float) this->options->getShellSize() / 2.0f, rect_movement.y + rect_movement.h, (float) this->options->getShellSize(), (float) this->options->getShellSize()};
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::KEYBOARD_CONFIRM], NULL, &rect_confirm);
    rect_confirm.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::MOUSE_CONFIRM], NULL, &rect_confirm);
    rect_confirm.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::GAMEPAD_CONFIRM], NULL, &rect_confirm);

    SDL_FRect rect_cancel_text = {(float) this->options->getScreenWidth() / 2.0f - (float) this->options->getShellSize() / 2.0f, rect_confirm.y + rect_confirm.h, 0.0f, 0.0f};
    SDL_GetTextureSize(this->text_cancel, &rect_cancel_text.w, &rect_cancel_text.h);
    rect_cancel_text.x -= rect_cancel_text.w;
    rect_cancel_text.y = rect_cancel_text.y - rect_cancel_text.h / 2.0f + (float) this->options->getShellSize() / 2.0f;
    SDL_RenderTexture(renderer, this->text_cancel, NULL, &rect_cancel_text);

    SDL_FRect rect_cancel = {(float) this->options->getScreenWidth() / 2.0f - (float) this->options->getShellSize() / 2.0f, (float) rect_confirm.y + (float) rect_confirm.h, (float) this->options->getShellSize(), (float) this->options->getShellSize()};
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::KEYBOARD_CANCEL], NULL, &rect_cancel);
    rect_cancel.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::MOUSE_CANCEL], NULL, &rect_cancel);
    rect_cancel.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::GAMEPAD_CANCEL], NULL, &rect_cancel);

    SDL_FRect rect_menu_text = {(float) this->options->getScreenWidth() / 2.0f - (float) this->options->getShellSize() / 2.0f, (float) rect_cancel.y + (float) rect_cancel.h, 0, 0};
    SDL_GetTextureSize(this->text_menu, &rect_menu_text.w, &rect_menu_text.h);
    rect_menu_text.x -= rect_menu_text.w;
    rect_menu_text.y = rect_menu_text.y - rect_menu_text.h / 2 + (float) this->options->getShellSize() / 2.0f;
    SDL_RenderTexture(renderer, this->text_menu, NULL, &rect_menu_text);

    SDL_FRect rect_menu = {(float) this->options->getScreenWidth() / 2.0f - (float) this->options->getShellSize() / 2.0f,  (float) rect_cancel.y + (float) rect_cancel.h, (float) this->options->getShellSize(), (float) this->options->getShellSize()};
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::KEYBOARD_MENU], NULL, &rect_menu);
    rect_menu.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::MOUSE_MENU], NULL, &rect_menu);
    rect_menu.x += (float) this->options->getShellSize();
    SDL_RenderTexture(renderer, this->button_images[(int) ButtonImage::GAMEPAD_MENU], NULL, &rect_menu);
}
