...
void ModeSelectState::draw(SDL_Renderer * renderer) {
    ...
    for(int i = 0; i < (int) texts.size(); i++) {
        ...
        SDL_FRect rect = {(float) this->options->getScreenWidth() / 2.0f, (float) getOptionY(i), 0.0f, 0.0f};
        ...
        sub_rect.h = 0.0f;
        if (sub_texts[i] != NULL) {
            sub_rect = {(float) this->options->getScreenWidth() / 2.0f, rect.y+rect.h, 0.0f, 0.0f};
        ...
        if(i == selection) {
            // Draw selection box
            SDL_FRect rect_selection = {0.0f, rect.y, (float) this->options->getScreenWidth(), rect.h + sub_rect.h};
